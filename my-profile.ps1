...
function Invoke-Update {
    ...
    # Check if the script already exists in the profile file
    if (-not (Select-String -Path $profile -Pattern ". $customProfilePath" -Quiet)) {
        # Add the script to the profile
        Add-Content -Path $profilePath -Value ". $customProfilePath"
        Write-Host "The script has been added to your profile."
    }
    else {
        Write-Host "The script is already in your profile."
    }
}

Set-Alias -Name "Invoke-Update" -Value Invoke-Update
